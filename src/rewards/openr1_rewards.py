...
# Commented out imports that require utils module
...
def equation_reward(completions, target, nums, **kwargs):
    """
    Evaluates completions based on mathematical correctness of the answer for countdown tasks.
 
    Args:
        completions (list[list[dict]]): Generated outputs in TRL format
        target (list[str]): Expected answers
        nums (list[list[int]]): Available numbers for each problem
    ...
    for completion, gt, numbers in zip(completion_contents, target, nums):
        try:
            # 0) pull <answer> ... </answer>
            ...
            # 3) check that the **left-hand side** used the given numbers once
            lhs_numbers   = [int(n) for n in re.findall(r'\d+', parts[0])]
            expected_nums = list(numbers)          # ← whatever your task supplies
            if sorted(lhs_numbers) != sorted(expected_nums):
                rewards.append(0.0)
                continue

            # 4) evaluate each chunk in a stripped namespace
            sandbox = {"__builtins__": None}
            values  = [eval(expr, sandbox, {}) for expr in parts]

            all_equal  = all(abs(v - values[0]) < 1e-5 for v in values[1:])
            matches_gt = abs(values[0] - float(gt)) < 1e-5

            rewards.append(1.0 if all_equal and matches_gt else 0.0)

        except Exception as err:
            print("grader error:", err)   # or logging
            rewards.append(0.0)
...
# Commented out - requires utils module
